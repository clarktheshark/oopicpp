...
void tenantInputScreen::getTenant() // get tenant info
{
  ...
  getaLine(tName);
  cout << "Enter tenant's aprtment number (101): ";
  cin >> aptNo;
  cin.ignore(80, '\n'); // make tenant
...
rentRecord::~rentRecord() // destructor
{
  while( !setPtrsRR.empty() ) // delete rent rows,
    {
      iter = setPtrsRR.begin(); // remove ptrs from set
      delete *iter;
...
void rentRecord::insertRent(int aptNo, int month, float amount)
{
  ...
  setPtrsRR.insert(ptrRow); // put rent in row
} // end insertRent()
//---------------------------------------------------------
...
void expenseRecord::display()
{
  ...
  if( vectPtrsExpenses.size() == 0)
    ...
    {
      sort( vectPtrsExpenses.begin(), // sort by data
      ...
      else if (ch=='d') // display data
	{
	  ...
	  switch(ch)
	    ...
	    case 'a':
	      ...
	      break;
	    default: cout << "Unknown display options\n";
	      break;
	    } // end switch
	} // end elseif
      else if (ch=='q')
