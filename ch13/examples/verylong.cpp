...
#include "verylong.h" // header file for very long
void strrev(char* s)
{
  char temp;
...
void verylong::putvl() const // display verylong
{
  ...
  strrev(temp); // reverse it
  cout << temp; // then display it
}
//------------------------------------------------
...
verylong verylong::mult10(const verylong v) const // multiply arg by 10
{
  ...
  temp[0] = '0'; // put zero in low end
  temp[v.vlen+1] = '\0'; // terminate string
  return verylong(temp); // return result
}
//------------------------------------------------------------
verylong verylong::multdigit(const int d2) const // multiply this verylong by digit in arg
{
  char temp[SZ];
  int j, carry = 0;
  for(j = 0; j < vlen; j++) // for each position
    {
      ...
      digitprod += carry; // add old varry
      if(digitprod >= 10) // if there's a new varry
	{
