...
void building::master_tick() //master time tick
{
  ...
  for(j = 0; j < NUM_CARS; j++) // for each elevator
    car_list[j]->car_tick2(); // send it time tick2
} // end master_tick()
//------------------------------------------------------
...
void building::record_floor_reqs()
{
  ...
  //return;
  //cin.ignore(10, '\n');
  if(ch=='\x1B') // if escapre key, end program
    exit(0);
...
//-------------------------------------------------------------
// get_cars_floor() --find which floor car is on?
...
//--------------------------------------------------------------
void elevator::car_display() // dipslay elevator image
{
  ...
  switch(loading_timer)
    {
    case 3:
      cout << "\u263a\xDB \xDB "; // draw car with open door // happy face on left
      break;
    case 2:
      ...
      break;
    case 1:
      cout << " \xDB\xDB\xDB "; // draw with closed door no happy face
      break;
    case 0:
      cout << " \u263a\xDB\xDB: "; // closed door , no happy face(default)
      break;
    }
  ...
  switch(unloading_timer)
    ...
    case 3:
      ...
      break;
    case 2:
      cout << "\xDB \xDB\x01"; // draw car with open door happy face on right\
      break;
    case 1:
      cout << "\xDB\xDB\xDB "; // draw with closed door no happy face
      break;
    case 0:
      cout << "\u25aa\u25aa\u25aa "; // closed door no happy face (default)
      break;
    }
...
void elevator::decide() // decide what to do
{
  ...
  // floor and directino of other car (not us)
  int ofloor; // floor
  ...
  // right way, go towards destinations and jesus
  ...
  for(j = 0; j < NUM_CARS; j++) // check each car
	...
	{ // get it's floor
	  ...
	  if( (odir==UP || odir==STOP) && requests_above )
		...
		// or on the same floor as us but is lower car number
  ...
  // if nothering else happening, stop
  current_dir = STOP;
} // end decide()
//-------------------------------------------------------------
void elevator::move()
...
void elevator::get_destinations() // stop, get destinations
{
  ...
  cout << "Car " << (car_number+1)
       ...
       << "\nEnter destination floors (0 when finiished)";
